...
class SendController extends ChangeNotifier {
  ...
  Future<void> pickPhotos({
    ...
    try {
      ...
      if (error is PlatformException && error.code == 'picker_launch_failed') {
        ...
        errorMessage = _toUserMessage(error);
      }
    } finally {
      isBusy = false;
      notifyListeners();
  ...
  Future<void> startAutoSending({
    ...
    if (!canUseAutoMode) {
      errorMessage =
          'Автоотправка недоступна. Проверьте настройки отправки и вход в Яндекс.';
      notifyListeners();
      return;
    }
    final parsedLimit = limitBytes;
  ...
  Future<void> refreshAutoJobStatus({bool notify = true}) async {
    ...
    try {
      ...
      if (status.isTerminal) {
        if (status.state == 'succeeded') {
          ...
          errorMessage = _toUserMessage(status.lastError ?? 'failed');
        }
      }
    } catch (error) {
      errorMessage = _toUserMessage(error);
  ...
  List<PhotoDescriptor> _orderedPhotosForSending(
    ...
    final ordered = List<PhotoDescriptor>.from(source);
    switch (sendOrderOptionFromId(settings.sendOrder)) {
      ...
      case SendOrderOption.sizeAsc:
        ordered.sort((a, b) {
          final bySize = a.sizeBytes.compareTo(b.sizeBytes);
          if (bySize != 0) {
            return bySize;
          }
  ...
  String _toUserMessage(Object error) {
    if (error is PlatformException) {
      ...
      if (code.contains('yandex_auth') || code == 'unauthorized') {
        return 'Не удалось войти в Яндекс. Проверьте аккаунт и разрешения.';
      }
      if (code.contains('network') ||
          code.contains('timeout') ||
          code.contains('host')) {
        return 'Нет интернета. Попробуйте позже.';
      }
      if (code.contains('smtp_auth') ||
          code.contains('authentication') ||
          code.contains('invalid_token')) {
        return 'Письмо не отправлено. Проверьте токен/сессию Яндекса.';
      }
      if (code == 'picker_launch_failed') {
        return 'Не удалось открыть галерею. Попробуйте позже.';
      }

      if (details.toLowerCase().contains('authenticationfailedexception')) {
        return 'Письмо не отправлено. Проверьте токен/сессию Яндекса.';
      }
      if (details.toLowerCase().contains('network') ||
          details.toLowerCase().contains('socket') ||
          details.toLowerCase().contains('timeout') ||
          details.toLowerCase().contains('host')) {
        return 'Нет интернета. Попробуйте позже.';
      }
    }
    ...
    if (normalized.contains('authenticationfailedexception') ||
        normalized.contains('smtp_auth_failed') ||
    ...
    if (normalized.contains('yandex_auth_failed') ||
